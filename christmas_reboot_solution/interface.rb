...
until action == 'quit'
  puts 'Which action [list|add|delete|mark|quit]' # add the mark option
  ...
  when 'mark'
    ...
    puts 'Which gift would you like to mark as bought?(Select Index):'
    print '>'
    ...
    list(gift_list)
  when 'idea'
    # Ask user which item he is searching for
    puts "What are you searching for"
    # Get user input, and store it in a variable
    user_query = gets.chomp
    # Call the scrape method
    ideas_array = scrape_etsy(user_query)
    # Display the ideas array
    list(ideas_array)
    # Ask the user to add one of the ideas
    user_index = gets.chomp.to_i - 1
    # Push the idea in our gift_list array
    gift_list << ideas_array[user_index]
  else
    puts 'Wrong input by the user'
  end
end
